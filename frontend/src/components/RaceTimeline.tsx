  ...
  return (
    <TooltipProvider>
      <Card className="bg-[var(--bg-card)] backdrop-blur-xl border-[var(--f1-border)] rounded-2xl overflow-hidden gap-0 py-0 shadow-none">
        <CardHeader className="px-5 py-3 border-b border-[var(--f1-border)] gap-0 pb-3">
                ...
                <Tooltip key={i}>
                  <TooltipTrigger asChild>
                    <motion.div
                      ...
                      className={`absolute top-0 h-full w-1 ${EVENT_COLORS[event.category] || "bg-neutral-500"}`}
                      style={{ left: `${pct}%` }}
                    />
                  </TooltipTrigger>
        ...
        <CardContent className="px-5 pb-4 pt-0 flex flex-wrap gap-2">
